...
class MVPDataset(data.Dataset):
    def __init__(self, train=True, npoints=2048, novel_input=True, novel_input_only=False):
        if train:
            self.input_path = 'MVP/mvp_train_input.h5'
            self.gt_path = 'MVP/mvp_train_gt_%dpts.h5' % npoints
        else:
            self.input_path = 'MVP/mvp_test_input.h5'
            self.gt_path = 'MVP/mvp_test_gt_%dpts.h5' % npoints
    ...
    def __getitem__(self, index):
        ...
        partial = Data(pos=partial)
        complete = Data(pos=complete)
